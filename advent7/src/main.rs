...
impl PartialOrd for Card {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        ...
        other_idx.partial_cmp(&self_idx)
    }
}
...
impl Hand {
    ...
    fn four_of_a_kind(&self) -> bool {
        ...
        for card in &self.cards {
            let count = counts.get(card).unwrap_or(&0);
            counts.insert(*card, count + 1);
        }
        counts.values().collect::<Vec<_>>().contains(&&4)
    }
    ...
    fn  three_of_a_kind(&self) -> bool {
        ...
        counts.values().collect::<Vec<_>>().contains(&&3)
    }
...
impl Hand2 {
    ...
    fn five_of_a_kind(&self) -> bool {
        ...
        for key in counts.keys() {
            if counts.get(key).unwrap() + counts.get(&Card2::new('J')).unwrap_or(&0) == 5 {
                return true;
            }
        }
        if *counts.get(&Card2::new('J')).unwrap_or(&0) == 5 {
            return true;
        }
        false
    }
    ...
    fn four_of_a_kind(&self) -> bool {
        ...
        for key in counts.keys() {
            if key != &Card2::new('J') && counts.get(key).unwrap() + counts.get(&Card2::new('J')).unwrap_or(&0) == 4 {
                return true;
            }
        }
        false
    ...
    fn  three_of_a_kind(&self) -> bool {
        ...
        for key in counts.keys() {
            if counts.get(key).unwrap() + counts.get(&Card2::new('J')).unwrap_or(&0) == 3 {
                return true;
            }
        }
        false
    ...
    fn full_house(&self) -> bool {
        ...
        card_count == 2 || (card_count == 3 && counts.get(&Card2::new('J')).is_some())
    }
    ...
    fn one_pair(&self) -> bool {
        ...
        let pair_count = counts.values().filter(|val| **val == 2).count();
        pair_count == 1 || *counts.get(&Card2::new('J')).unwrap_or(&0) > 0
    }
...
fn part_one(lines: &InputType) -> ResultType {
    ...
    for (i, (_, bet)) in lines.iter().enumerate() {
